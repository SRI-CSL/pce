...
/*
 * Deep copy of the atom table:
 *
 * Need to pass in the pred_table to be able to look up predicates and
 * check their arity.

 * Functionally equivalent to atom_copy( samp_atom_t *atom, int32_t
 * arity), but with a slightly different signature & internals:
 */
...
/*
 * Deep copy of the sample table - there are two motivations for this:
 * 1) for allowing us to run multithreaded mcmc, and 2) to "pickle"
 * the tables and distribute them over a cluster / cloud.  The idea is
