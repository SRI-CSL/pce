...
bool member_frozen_preds(samp_literal_t lit, int32_t *frozen_preds, samp_table_t *table) {
  atom_table_t *atom_table = &table->atom_table;
  samp_atom_t *atom;
  int32_t i, lit_pred;
...
rule_atom_t * typecheck_atom(input_atom_t *atom,
			     samp_rule_t *rule,
  ...
  new_atom->args = (rule_atom_arg_t *)
    safe_malloc(pred_entry.arity * sizeof(rule_atom_arg_t));
...
bool eql_rule_atom(rule_atom_t *atom1, rule_atom_t *atom2,
		   samp_table_t *table) {
  int32_t i, arity;
...
bool eql_rule_literal(rule_literal_t *lit1, rule_literal_t *lit2,
  ...
  return ((lit1->neg == lit2->neg)
	  && eql_rule_atom(lit1->atom, lit2->atom, table));
}
...
int32_t negative_unit_propagate(samp_table_t *table){
  ...
  while (link != NULL) {
	...
	if (conflict == -1) {
	  ...
	  cprintf(1,"negative_unit_propagate: conflict fixing lit %s to true: ",
		  litstr);
	  free(litstr);
	  return -1;
	} else {
...
void init_random_assignment(samp_truth_value_t *assignment, int32_t num_vars,
  ...
  *num_unfixed_vars = 0;
  ...
  for (i = 0; i < num_vars; i++){
	...
	cprintf(1,"init_random_assignment: assigned false to var %d\n", i);
      } else {
	assignment[i] = v_true;
...
int32_t choose_clause_var(samp_table_t *table,
  ...
  for (i = 0; i < link->numlits; i++){
    if (!link->frozen[i] &&
	!fixed_tval(assignment[var_of(link->disjunct[i])]))
...
int32_t sample_sat_body(samp_table_t *table, double sa_probability,
  ...
  if (clause_table->num_unsat_clauses <= 0 || choice < sa_probability) {
    ...
    cprintf(1," in simulated annealing: num_unsat = %d, var = %d, dcost = %d\n",
...
void mc_sat(samp_table_t *table, uint32_t max_samples,
  ...
  for (i = 0; i < max_samples; i++){
    ...
    if (get_verbosity_level() > 0) print_assignment(table);
    //print_assignment(table);
...
void new_const_atoms_at(int32_t idx, int32_t newcidx, int32_t arity, int32_t *psig,
...
int32_t substit_rule(samp_rule_t *rule,
  ...
  for(i=0; i<rule->num_lits; i++) {
	...
	found_indirect = true;
      ...
      safe_free(new_atom);
      //}
      clause_buffer.data[litidx++]
	= lit->neg ? neg_lit(added_atom) : pos_lit(added_atom);
    }
  }
  return add_internal_clause(table, clause_buffer.data, litidx,
			     rule->frozen_preds, rule->weight,
			     found_indirect, false);
}
...
int32_t substit_query(samp_query_t *query,
  ...
  int32_t i, j, k, vsort, csort, csubst, arity, argidx, added_atom, numlits, litnum;
  rule_literal_t ***lit;
  ...
  for (i = 0; i < query->num_clauses; i++) {
	...
	safe_malloc((numlits + 1) * sizeof(samp_literal_t));
      litnum = 0;
      for (j = 0; lit[i][j] != NULL; j++) {
	if (lit[i][j]->atom->builtinop == 0) {
	  ...
	  free_samp_atom(new_atom);
	  litinst[i][litnum++] = lit[i][j]->neg ? neg_lit(added_atom) : pos_lit(added_atom);
	}
      }
