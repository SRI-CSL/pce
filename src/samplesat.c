...
void all_rule_instances_rec(int32_t vidx, samp_rule_t *rule,
...
static void add_atom_to_pred(pred_table_t *pred_table, int32_t predicate,
	...
	if (entry->num_atoms >= entry->size_atoms) {
		if (entry->size_atoms == 0) {
			entry->atoms = (int32_t *) safe_malloc(
			...
			size += size / 2;
			entry->atoms = (int32_t *) safe_realloc(entry->atoms,
					size * sizeof(int32_t));
...
void add_rule_to_pred(pred_table_t *pred_table, int32_t predicate,
	...
	if (entry->num_rules >= entry->size_rules) {
		if (entry->size_rules == 0) {
			entry->rules = (int32_t *) safe_malloc(
			...
			size += size / 2;
			entry->rules = (int32_t *) safe_realloc(entry->rules,
					size * sizeof(int32_t));
...
int32_t add_internal_clause(samp_table_t *table, int32_t *clause,
		int32_t length, int32_t *frozen_preds, double weight, bool indirect,
		bool add_weights) {
	atom_table_t *atom_table = &table->atom_table;
	uint32_t i, posidx;
	...
	if (indirect) {
		...
		clause_map = array_size_hmap_find(&(clause_table->clause_hash), length,
				(int32_t *) clause);
		if (clause_map == NULL) {
			...
			entry = (samp_clause_t *) safe_malloc(
					sizeof(samp_clause_t) + sizeof(bool *) + length
							* sizeof(int32_t));
			entry->frozen = (bool *) safe_malloc(length * sizeof(bool));
			...
			entry->link = NULL;
			for (i = 0; i < length; i++) {
				entry->frozen[i] = member_frozen_preds(clause[i], frozen_preds,
						table);
				entry->disjunct[i] = clause[i];
			}
			clause_map = array_size_hmap_get(&clause_table->clause_hash,
					length, (int32_t *) entry->disjunct);
...
int32_t fix_lit_true(samp_table_t *table, int32_t lit) {
	...
	if (fixed_tval(assignment[var])) {
		if (sign ? assigned_true(assignment[var]) : assigned_false(
				assignment[var])) {
...
int32_t fix_lit_false(samp_table_t *table, int32_t lit) {
	...
	if (fixed_tval(assignment[var])) {
		if (sign ? assigned_false(assignment[var]) : assigned_true(
				assignment[var]))
		...
		if (abs_weight == DBL_MAX || choose() > exp(-abs_weight)) {
			...
			if (link->weight >= 0) { //must be a unit clause
				...
				if (conflict == -1) {
					...
					cprintf(
							1,
							"negative_unit_propagate: conflict fixing lit %s to true: ",
							litstr);
					free(litstr);
					return -1;
				} else {
					cprintf(1, "negative_unit_propagate: Picking unit clause\n");
				}
...
void init_random_assignment(samp_truth_value_t *assignment, int32_t num_vars,
	...
	*num_unfixed_vars = 0;
	num = num_vars;
	cprintf(1, "init_random_assignment: num_vars = %d\n", num);
	for (i = 0; i < num_vars; i++) {
		if (!fixed_tval(assignment[i])) {
			if (choose() < 0.5) {
				assignment[i] = v_false;
				cprintf(1,
						"init_random_assignment: assigned false to var %d\n", i);
			} else {
				assignment[i] = v_true;
				cprintf(1, "init_random_assignment: assigned true to var %d\n",
						i);
			}
			(*num_unfixed_vars)++;
		} else {
...
void push_negative_unit_clause(clause_table_t *clause_table, uint32_t i) {
	clause_table->samp_clauses[i]->link
			= clause_table->negative_or_unit_clauses;
...
void empty_clause_lists(samp_table_t *table) {
	...
	for (i = 0; i < atom_table->num_vars; i++) {
		if (!fixed_tval(
				atom_table->assignment[atom_table->current_assignment][i])) {
			num_unfixed++;
		}
	...
	if (choice < rvar_probability) {//flip a random unfixed variable
		...
		for (i = 0; i < length_integer_stack(&clause_var_stack); i++) {
			cost_flip_unfixed_variable(
					...
					&vcost,
					var_of(
							link->disjunct[nth_integer_stack(i,
									&clause_var_stack)]));
			if (vcost < dcost) {
				dcost = vcost;
				vcost = 0;
...
void sample_sat(samp_table_t *table, double sa_probability,
	...
	while (num_flips > 0 && conflict == 0) {
		...
		conflict = sample_sat_body(table, sa_probability, samp_temperature,
				rvar_probability);
		//assert(valid_table(table));
		num_flips--;
	}
...
bool builtin_inst_p(rule_literal_t *lit, substit_entry_t *substs,
		samp_table_t *table) {
	pred_table_t *pred_table = &table->pred_table;
	...
	for (j = 0; j < arity; j++) {
		...
		if (lit->atom->args[j].kind == variable) {
			nargs[j] = substs[argidx].const_index;
		} else {
			nargs[j] = argidx;
		}
	}
	...
	safe_free(nargs);
	if (builtin_result) {
		return lit->neg ? false : true;
	} else {
		return lit->neg ? true : false;
	}
}
...
int32_t substit_rule(samp_rule_t *rule, substit_entry_t *substs,
		samp_table_t *table) {
	const_table_t *const_table = &table->const_table;
	sort_table_t *sort_table = &table->sort_table;
	// atom_table_t *atom_table = &table->atom_table;
	...
	for (i = 0; i < rule->num_vars; i++) {
		...
		if (csubst == INT32_MIN) {
			mcsat_err(
					"substit: Not enough constants - %"PRId32" given, %"PRId32" required\n",
					i, rule->num_vars);
			return -1;
		}
		...
		if (sort_entry->constants == NULL) {
			...
			if (!subsort_p(csort, vsort, sort_table)) {
				mcsat_err(
						"substit: Constant/variable sorts do not match at %"PRId32"\n",
						i);
				return -1;
			}
		}
	}
	if (substs != NULL && substs[i].const_index != INT32_MIN) {
		mcsat_err(
				"substit: Too many constants - %"PRId32" given, %"PRId32" required\n",
				i, rule->num_vars);
		return -1;
	}
	...
	for (i = 0; i < rule->num_lits; i++) {
		...
		if (lit->atom->builtinop != 0) {
			...
			// Not a builtin
			samp_atom_t *new_atom = rule_atom_to_samp_atom(lit->atom,
					pred_table);
			for (j = 0; j < arity; j++) {
 ...
 * flag is set.
 */
bool check_clause_instance(samp_table_t *table, samp_rule_t *rule,
	...
	for (i = 0; i < rule->num_lits; i++) {//for each literal
		...
		if (atom_map != NULL && atom_map->val == atom_index) {
			...
			if (atom_map == NULL || (neg ? assigned_false(
					assignment[atom_map->val]) : assigned_true(
					assignment[atom_map->val]))) {
				if (num_non_false == 0) {
					num_non_false = 1;
				} else {
					return false;
...
void all_rule_instances_rec(int32_t vidx, samp_rule_t *rule,
	...
	if (substit_buffer.entries[vidx].fixed) {
		...
		for (i = 0; i < entry.cardinality; i++) {
			...
			if (vidx == rule->num_vars - 1) {
				substit_buffer.entries[vidx + 1].const_index = INT32_MIN;
				if (!lazy_mcsat() || check_clause_instance(table, rule,
						atom_index)) {
					cprintf(2, "Rule is being activated\n");
					if (lazy_mcsat())
					{
						prev_num_clauses = clause_table->num_clauses;
					}
		...
		for (j = 0; j < rule->num_vars; j++) {
			if (subsort_p(csort, rule->vars[j]->sort_index, sort_table)) {
				...
				for (k = 0; k < rule->num_vars; k++) {
					if (k != j) {
						substit_buffer.entries[j].const_index = INT32_MIN;
...
int32_t add_subst_query_instance(samp_literal_t **litinst,
		substit_entry_t *substs, samp_query_t *query, samp_table_t *table) {
...
int32_t substit_query(samp_query_t *query, substit_entry_t *substs,
		samp_table_t *table) {
	const_table_t *const_table = &table->const_table;
	sort_table_t *sort_table = &table->sort_table;
	pred_table_t *pred_table = &table->pred_table;
		...
		if (numlits > 0) { // Can ignore clauses for which numlits == 0
			...
			for (j = 0; lit[i][j] != NULL; j++) {
				if (lit[i][j]->atom->builtinop == 0) {
					...
					new_atom = rule_atom_to_samp_atom(lit[i][j]->atom,
							pred_table);
					for (k = 0; k < arity; k++) {
						argidx = new_atom->args[k];
						if (lit[i][j]->atom->args[k].kind == variable) {
							// Have a var, index to var (and hence, const) array is -(argidx + 1)
							new_atom->args[k] = substs[argidx].const_index;
						}
					}
					if (lazy_mcsat()) {
						array_hmap_pair_t *atom_map;
			...
			for (k = 0; k < arity; k++) {//copy each instantiated argument
				if (atom->args[k].kind == variable) {
					rule_inst_atom->args[k]
							= substit_buffer.entries[atom->args[k].value].const_index;
				} else {
...
bool match_atom_in_rule_atom(samp_atom_t *atom, rule_literal_t *lit,
	...
	for (i = 0; i < substit_buffer.size; i++) {
		substit_buffer.entries[i].const_index = INT32_MIN;
		substit_buffer.entries[i].fixed = false;
	}
	// Now go through comparing the args of the atoms
	for (i = 0; i < arity; i++) {
		if (lit->atom->args[i].kind == constant || lit->atom->args[i].kind
				== integer) {
			...
			if (substit_buffer.entries[varidx].const_index == INT32_MIN) {
